...
W niniejszej pracy przedstawiony zostanie model Hestona, w który zmienność nie jest wartością stałą 
...
Do lat '90 XX wieku podstawowym modelem do wyceny opcji był model Blacka-Scholesa. Opublikowany w 1973 \cite{BlackScholes} roku artykuł, którego autorami byli Fischer Black oraz Myron Scholes, był przełomowy dla teorii wyceny opcji.
...
na globalnym rynku? Do najważniejszych z nich możemy zaliczyć:

\begin{enumerate}
  \item 
\end{enumerate}

\subsection{Opcje jako element zarządzania ryzykiem} % (fold)
...
\subsection{Opcje jako element zarządzania ryzykiem walutowym} % (fold)
...
\section{Numeryczne wyznaczenie wartości opcji} % (fold)
\label{sec:}
...
O ile schemat dyskretyzacji Eulera jest najprostszym koncepcyjnie, ma on wiele bra
...
\section{Algorytm Andersena} % (fold)
\label{sec:algorytm_andersena}

% section algorytm_andersena (end)
